...
pub(crate) fn generate_chunk_data_static(pos: crate::world::ChunkPosition) -> Chunk {
    ...
    if chunk_bottom_voxel > MAX_TERRAIN_VOXEL_HEIGHT || pos.y < 0 {
        return chunk;
    }
        ...
        for lz in 0..CHUNK_SIZE {
            ...
            for ly in 0..CHUNK_SIZE {
                ...
                let voxel = if height > 700 && depth_below_surface <= 2 {
                    ...
                    let snow_noise =
                        hash_2d(wx.wrapping_add(5000), wz.wrapping_add(6000));
                    ...
                    let rock_noise =
                        hash_2d(wx.wrapping_add(3000), wz.wrapping_add(4000));
                    if rock_noise < 0.5 {
                        42
                    }
                    // Stone gray
                    ...
                    let grass_noise =
                        hash_2d(wx.wrapping_add(1000), wz.wrapping_add(2000));
                    if grass_noise < 0.33 {
                        49
                    }
                    // Grass green
                    ...
                    let deep_noise =
                        hash_2d(wx.wrapping_add(7000), wz.wrapping_add(8000));
...
pub(crate) fn generate_column_data_static(col: ColumnPos) -> Column {
    ...
    for sy in 0..NUM_SECTIONS as i32 {
        ...
        let chunk = generate_chunk_data_static(pos);
        if !chunk.is_empty() {
...
pub(crate) fn terrain_height(wx: i32, wz: i32) -> i32 {
    ...
    let continental = fbm(x * 0.0003, z * 0.0003, 2);
    ...
    let ridge_raw = fbm(x * 0.0012 + 500.0, z * 0.0012 + 700.0, 4);
    let ridges = (1.0 - ridge_raw.abs()) * (1.0 - ridge_raw.abs()); // squared ridged noise

    // Large rolling hills
    let h1 = fbm(x * 0.0008, z * 0.0008, 4) * 200.0;
    // Medium terrain variation
    let h2 = fbm(x * 0.003 + 100.0, z * 0.003 + 200.0, 4) * 80.0;
    // Small bumps and detail
    let h3 = fbm(x * 0.012 + 300.0, z * 0.012 + 400.0, 3) * 20.0;
    // Fine detail
    let h4 = fbm(x * 0.05 + 600.0, z * 0.05 + 800.0, 2) * 5.0;
    ...
    height.clamp(16.0, MAX_TERRAIN_VOXEL_HEIGHT as f32) as i32
}
...
pub(crate) fn generate_lod_data_static(key: &LodNodeKey) -> Option<world::lod::VoxelData> {
    ...
    if origin_vy > MAX_TERRAIN_VOXEL_HEIGHT {
        return None;
    }
